...
Combining the pipeline, the wide kernels and joint training over the three
datasets reaches 91.5\% test accuracy with 3.4M parameters and 5.9 GFLOPs
per view. Nine RWF-2000 training clips turn out to be copies of test clips.
\end{abstract}
...
\begin{itemize}
  ...
  \item A model that reaches 91.5\% on RWF-2000 with 3.4M parameters and
...
Two properties make the module practical rather than merely accurate. It
is structurally reparameterised: the pre-trained $3\times3$ kernel and the
zero-initialised outer ring are separate parameters during training, in
separate learning-rate groups, and fold into one $5\times5$ kernel for
inference, so the deployed graph is a plain X3D with slightly larger
depthwise convolutions and 3.27M parameters rather than the 3.44M that
train. Measured on one batch in fp32, the fused and unfused models agree to
$3.7\times10^{-7}$. And because the ring starts at zero, the network begins
fine-tuning as exactly the Kinetics model it extends, which matters at this
data scale: the same widening trained as a dense $5\times5$ kernel from the
inflated weights is worth 0.19 points less.
...
Table~\ref{tab:final} puts the three ingredients together on the full
training split, with no epoch selection of any kind: the schedule runs to
its end and the test split is scored once. The uncropped pipeline, the
all-stage wide kernels and joint training over RWF-2000, Hockey Fights and
RLVS give 91.5\% with 3.4M parameters and 5.9 GFLOPs per view. For
comparison, the same network and recipe on the pipeline of the original
work reaches 86.5\%.

\begin{table}[t]
  \centering
  \caption{RWF-2000, trained on the whole training split, test split scored
  once. Joint training adds the training clips of Hockey Fights and RLVS.}
  \label{tab:final}
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{lccccc}
    \toprule
    Model & Pipeline & Joint & Test & F1 & AUC \\
    \midrule
    MA-X3D         & cropped   & no  & 86.50 & 0.865 & 0.947 \\
    MA-X3D         & cropped   & yes & 88.00 & 0.881 & 0.951 \\
    MA-X3D         & uncropped & yes & 91.25 & 0.912 & 0.973 \\
    Wide kernels   & uncropped & yes & \textbf{91.50} & 0.915 & 0.972 \\
    \bottomrule
  \end{tabular}
\end{table}

Test-time augmentation is available at a price: averaging two temporal
offsets and their horizontal flips raises the four-fold ensemble from
89.50\% to 91.00\% on the same models, at four times the inference cost.
We report the single-view number as the headline, since the argument of
this paper is about what a compact model can do per GFLOP.
...
The resulting detector reaches 91.5\% on RWF-2000 with 3.4M parameters and
5.9 GFLOPs per view, trained on the full split with no selection of any
kind. We release the splits, the duplicate audit, the seven rebuilt
