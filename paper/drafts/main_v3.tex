...
it costs two test points, Section~\ref{sec:recipe-clean}). A dense
$5\times5$ kernel in a frozen stage is frozen as a whole, whereas the
separate ring keeps training while the pre-trained kernel of Res4 stays
fixed, so the widening reaches every stage without unfreezing any of them.
Where both forms can train, on the cropped pipeline with Res2--Res3
widened, the dense and reparameterised kernels are within noise (90.90 and
91.03 out of fold), so we present the reparameterisation as what makes the
all-stage widening compatible with a frozen stage, not as a source of
accuracy on its own.
