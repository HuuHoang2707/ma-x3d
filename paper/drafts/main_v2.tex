...
Under this protocol, and averaged over three seeds, the modules add 0.88
out-of-fold points when the backbone is fine-tuned conventionally
($[+0.04, +1.71]$) and nothing once the training recipe is tuned
($-0.04$ points on RWF-2000, $-0.04$ on Hockey Fights, $-0.19$ on RLVS),
including when only a quarter of the training clips is used. The recipe
itself adds 2.80 points to the backbone, and two effects outside the model
are of the same order as the modules: choosing the epoch on the test split
adds 0.8 points on average, and one seed made a $+1.25$-point gain on
Hockey Fights look significant that three seeds remove. Replacing the gate
by a module that \emph{adds} a motion-derived signal instead of rescaling
features, at 16K parameters, raises test accuracy from 87.4\% to 88.2\%. Six Kinetics backbones trained under the same
