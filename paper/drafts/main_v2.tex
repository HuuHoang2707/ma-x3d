...
Under this protocol the modules leave accuracy unchanged on all three
datasets once results are averaged over three seeds ($-0.04$ points on
RWF-2000, $p=0.78$; $-0.04$ on Hockey Fights; $-0.19$ on RLVS), including
when only a quarter of the training clips is used, while the training
recipe alone is worth 1.6 to 1.9 points. Single-seed runs tell a different
story: on Hockey Fights one seed shows a significant $+1.25$ points, which
three seeds remove. Six Kinetics backbones trained under the same
...
The outcome is consistent across the three datasets: averaged over three
seeds the modules change accuracy by $-0.04$ points on RWF-2000
($p=0.78$), $-0.04$ on Hockey Fights and $-0.19$ on RLVS, and by $0.00$
points when only a quarter of the training clips is used. The training
recipe, in contrast, adds 1.6 points to X3D-M and 1.9 to MA-X3D. Controls
explain part of the picture: a gate that keeps its parameters but receives
...
\begin{itemize}
  ...
  \item A study of two cheap architectural modules with matched controls,
        three seeds and three datasets, showing no measurable gain over a
        properly adapted backbone in any regime we tested, including
...
the same scene together, and never uses the test split for a decision. Averaged
over three seeds the additions leave accuracy unchanged on all three
datasets and in every training regime we tested, while the training recipe
is worth more than either module, and two effects outside the model, epoch
selection on the test split and the choice of seed, produce differences
larger than the modules do. Under
